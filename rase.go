/*
 List 3.5  Счетчик слов с состоянием гонки
	$ go run ./rase.go ./exampledata/*.txt
	Все горутины использую 1 общий объект структуру words что приводит к ошибку
Проблема возникла в функции words.add. Несколько сопрограмм
одновременно  обращаются  к  одному  и  тому  же  фрагменту  памяти,
объекту words.found типа map
*/
package main
